...
int main() {
    ...
    eng.register_state(exutil::rec(StateId(1), StateGeneration(1), "kv", "mem", 1, 0, exact), exutil::env(eng.epoch(), StateGeneration(1)));
    eng.register_state(exutil::rec(StateId(2), StateGeneration(1), "kv", "mem", 1, 0, compat), exutil::env(eng.epoch(), StateGeneration(1)));
    QueryDescriptor q; q.query_id = QueryId(6); q.kind = StateKind::KV_STATE; q.compatibility = CompatibilityRequirement::EXACT;
    auto r = eng.query(q);
    std::cout << "exact-only candidates=" << r.ranked_candidates.size() << " selected=" << (r.selected_record_id ? to_string(*r.selected_record_id) : "none") << "\n";
    return 0;
}
